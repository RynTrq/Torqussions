...
const Project = () => {
  ...
  const [assistantProvider, setAssistantProvider] = useState(defaultAssistantInfo.provider)
  const [assistantModel, setAssistantModel] = useState(defaultAssistantInfo.model)
  const [assistantSaveStatus, setAssistantSaveStatus] = useState('idle')
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    try {
      ...
      startTransition(() => {
        ...
        setAssistantProvider(nextAssistantInfo.provider || defaultAssistantInfo.provider)
        setAssistantModel(nextAssistantInfo.model || defaultAssistantInfo.model)
        setAssistantSaveError('')
        setAssistantSaveStatus('idle')
  ...
  const handleAssistantProviderChange = (provider) => {
    const nextProvider =
      assistantInfo.availableProviders?.find((candidate) => candidate.id === provider) || null

    setAssistantProvider(provider)
    setAssistantModel(nextProvider?.defaultModel || '')
    setAssistantSaveError('')
  }

  const handleAssistantSettingsSave = async () => {
    if (!project?.permissions?.canInvite) {
      return
    }

    setAssistantSaveStatus('loading')
    setAssistantSaveError('')

    try {
      const { data } = await axios.put(`/projects/${projectId}/assistant`, {
        provider: assistantProvider,
        model: assistantModel.trim(),
      })

      setProject(data.project)
      setAssistantInfo(data.assistant || defaultAssistantInfo)
      setAssistantProvider(data.assistant?.provider || assistantProvider)
      setAssistantModel(data.assistant?.model || assistantModel.trim())
      setNotice('Project AI settings updated.')
    } catch (requestError) {
      setAssistantSaveError(
        requestError.response?.data?.error ||
          ...
          <section className="mb-6 grid gap-6 2xl:grid-cols-[1.08fr,0.92fr]">
            <div className="torq-shell-strong rounded-[2rem] p-6 sm:p-8">
              <div className="flex flex-wrap gap-2">
                ...
                <div className="torq-badge border border-white/12 bg-white/10 text-white">
                  {assistantInfo?.providerLabel || 'AI'} • {assistantInfo?.model}
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              {activeWorkspacePanel === 'ai' ? (
                <AssistantSettingsPanel
                  assistantInfo={assistantInfo}
                  canManage={Boolean(project?.permissions?.canInvite)}
                  modelInput={assistantModel}
                  onModelInputChange={setAssistantModel}
                  onProviderChange={handleAssistantProviderChange}
                  onSave={handleAssistantSettingsSave}
                  providerValue={assistantProvider}
                  saveError={assistantSaveError}
